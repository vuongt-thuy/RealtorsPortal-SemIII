...
namespace RealtorsPortal.Controllers
{
    public class RentController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var categories = resCat.GetList(x => x.Active == true).Select(x => new CategoryMapper().Mapping(x));
            return View();
        }
        ...
        {
            var ads = resAds.GetAll().Where(x => x.Need == SystemConstant.NEED_RENT && x.Status == SystemConstant.APPROVED).Select(x => new AdsMapper().Mapping(x));
        ...
        {
            var ads = new AdsMapper().Mapping(resAds.FindById(id));
            if (ads.Need == SystemConstant.NEED_RENT)
            {
                var allAds = resAds.GetList(x => x.Need == SystemConstant.NEED_RENT && x.Status == SystemConstant.APPROVED && x.CategoryId == ads.CategoryId).ToList();
